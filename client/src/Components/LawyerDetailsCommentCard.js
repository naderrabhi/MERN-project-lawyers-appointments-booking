...
const LawyerDetailsCommentCard = ({post, avocatID, User, toggleEdit, handleEdit, setCommentEdit}) => {
  ...
  return (
    <div className="lawyerDetailsCommentList--comment">
            ...
            {toggleEdit ? <textarea onChange={(e) => setCommentEdit(e.target.value)} Value={post.comment && post.comment} name="comment" id="comment" cols="30" rows="10" className="textarea--custom" /> : <p>{post.comment}</p> }
